---
title: 'Lync Server 2013: Ejecución de pruebas de enrutamiento de voz'
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
f1.keywords:
- NOCSH
TOCTitle: Running voice routing tests
ms:assetid: 577cdc57-930e-4e12-a515-fdcf61b93153
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Gg398377(v=OCS.15)
ms:contentKeyID: 48184185
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: 869966f5d315fdfaa92409a7586b9eec8fd01ec4
ms.sourcegitcommit: b693d5923d6240cbb865241a5750963423a4b33e
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 02/04/2020
ms.locfileid: "41765038"
---
<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="http://msdn.microsoft.com/en-us/">

<div data-asp="http://msdn2.microsoft.com/asp">

# <a name="running-voice-routing-tests-in-lync-server-2013"></a>Ejecución de pruebas de enrutamiento de voz en Lync Server 2013

</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**Última modificación del tema:** 2013-02-21_

Lync Server 2013 ofrece dos métodos diferentes para probar las rutas de voz: puede realizar una prueba informal y ad hoc con cualquier número de teléfono y cualquier ruta de voz; o bien, puede realizar pruebas más formales con los casos de prueba de ruta de voz. Con las pruebas formales, se define como el número que se marcará y el plan de marcado y la Directiva de voz que se utilizará, de modo que Lync Server pueda comprobar que, dadas estas condiciones, el número proporcionado se pueda enrutar correctamente a la red PSTN. Ambos métodos se describen en secciones posteriores de esta documentación.

<div>

## <a name="in-this-section"></a>En esta sección

  - [Ejecutar pruebas de enrutamiento de voz informal en Lync Server 2013](lync-server-2013-run-informal-voice-routing-tests.md)

  - [Ejecutar los casos de prueba de enrutamiento de voz en Lync Server 2013](lync-server-2013-run-voice-routing-test-cases.md)

</div>

</div>

<span> </span>

</div>

</div>

</div>

