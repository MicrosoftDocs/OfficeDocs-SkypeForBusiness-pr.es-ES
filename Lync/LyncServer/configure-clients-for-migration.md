---
title: Configurar los clientes para la migración
description: Configurar los clientes para la migración.
ms.reviewer: ''
ms.author: serdars
author: serdarsoysal
audience: Admin
f1.keywords:
- NOCSH
TOCTitle: Configure clients for migration
ms:assetid: ca796a3d-9705-43a5-a6c4-09da9923b5cc
ms:mtpsurl: https://technet.microsoft.com/en-us/library/JJ205263(v=OCS.15)
ms:contentKeyID: 48185411
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: da1aa7140cf38f8e8a7002ed748ef77d1c2ea262
ms.sourcegitcommit: d42a21b194f4a45e828188e04b25c1ce28a5d1ae
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 10/17/2020
ms.locfileid: "48548506"
---
# <a name="configure-clients-for-migration"></a>Configurar los clientes para la migración

<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="https://msdn.microsoft.com/">

<div data-asp="https://msdn2.microsoft.com/asp">



</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**Última modificación del tema:** 2012-09-24_

Este tema contiene los pasos de implementación de cliente recomendados que debe realizar antes de migrar a Lync Server 2013. Estos cambios de configuración deben realizarse en Lync Server 2010.

<div>

## <a name="to-configure-clients-before-migration"></a>Para configurar los clientes antes de la migración:

1.  Implemente las actualizaciones más recientes de servidor, cliente y dispositivo (hotfixes) para Lync Server 2010. Para obtener la información más actualizada sobre Lync Server 2010, consulte actualizaciones del centro de recursos para Lync en <https://go.microsoft.com/fwlink/p/?linkid=232630> .

2.  En Lync Server 2010, use el filtrado de versiones de cliente para permitir solo a los clientes que tengan instaladas las actualizaciones más recientes.

</div>

<div>

## <a name="see-also"></a>Consulte también


[Configuración nueva y modificada para Lync 2013](lync-server-2013-new-and-changed-settings-for-lync-2013.md)  
[Interoperabilidad de clientes en Lync 2013](lync-server-2013-client-interoperability-in-lync-2013.md)  


[Planeación de clientes y dispositivos en Lync Server 2013](lync-server-2013-planning-for-clients-and-devices.md)  
  

</div>

</div>

<span> </span>

</div>

</div>

</div>

