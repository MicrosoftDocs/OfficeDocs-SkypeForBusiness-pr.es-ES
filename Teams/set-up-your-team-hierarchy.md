...
> [!NOTE]
> Si un nodo no es un nodo raíz o un nodo hoja y no necesita la pertenencia al equipo para conceder los permisos correspondientes de publicación e informes, puede dejar el TeamId en blanco. Este método se puede usar para agregar más granularidad al elegir equipos de destinatarios o para ver informes de finalización sin tener un equipo correspondiente.
...
Puede agregar columnas de cubo para crear cubos, que son agrupaciones en las que se pueden organizar las tareas. Cada cubo obtiene su propia columna en el archivo CSV. Los cubos que cree estarán disponibles para el equipo de publicación. Después, el equipo de publicación puede usar estos cubos para clasificar las tareas de los equipos destinatarios. Si un cubo aún no existe en un equipo, los cubos se crean a petición cuando se publican las tareas.
...
Puede cargar una nueva jerarquía para reemplazar la anterior con el mismo comando de PowerShell que el anterior. Cada vez que cargue una nueva jerarquía, esta reemplaza a la jerarquía anterior.
...
2. Si la **columna Id.** de grupo no  se muestra en la tabla, seleccione Editar columnas en la esquina superior derecha de la tabla y active **Id. de grupo.**
