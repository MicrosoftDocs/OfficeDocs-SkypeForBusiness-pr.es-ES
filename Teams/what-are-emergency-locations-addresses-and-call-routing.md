...
Este artículo describe conceptos que necesita saber para administrar las llamadas de emergencia, incluyendo información sobre las direcciones de emergencia, las direcciones de emergencia dinámicas y el enrutamiento de llamadas de emergencia. En este artículo se usa la siguiente terminología:

- **Dirección de emergencia** , una dirección cívica, la dirección física o postal de un lugar de trabajo para su organización.
...
- **Ubicación de emergencia** : una ubicación es una dirección cívica, con un lugar opcional. Si tu empresa tiene más de una ubicación física, es probable que necesites más de una ubicación de emergencia. 

  Al crear una dirección de emergencia, se crea automáticamente un identificador de ubicación único para esta dirección.  Si agrega un lugar a una dirección de emergencia, por ejemplo, si agrega un piso a una dirección de creación, se crea un identificador de ubicación para la combinación de la dirección de emergencia y el lugar.  En este ejemplo, habrá dos identificadores de Ubicación: uno para la dirección cívica; una para la dirección de la ciudad y la ubicación asociada.
...
- Para otros países, como en Europa, Oriente Medio y África (EMEA), se necesita una ubicación de emergencia cuando obtiene el número de teléfono de Office 365 o cuando se transfiere desde otro proveedor u operador de servicios.
...
En los Estados Unidos, debe configurar la dirección cívica que forma parte de las ubicaciones de emergencia asignadas a identificadores de red e incluir los códigos geográficas asociados. Para obtener más información, consulte [planear y configurar las llamadas de emergencia dinámicas](configure-dynamic-emergency-calling.md).
...
Puede asignar una TeamsEmergencyCallRoutingPolicy a una cuenta de usuario de enrutamiento directo, a un sitio de red o a ambas. Cuando un cliente de equipo inicia o cambia una conexión de red, Teams realiza una búsqueda del sitio de red donde se encuentra el cliente de la siguiente manera:

- Si un TeamsEmergencyCallRoutingPolicy está asociado con el sitio, se usa la Directiva del sitio para configurar las llamadas de emergencia.

- Si no hay ningún TeamsEmergencyCallRoutingPolicy asociado al sitio, o si el cliente está conectado a un sitio no definido, se usa la TeamsEmergencyCallRoutingPolicy asociada a la cuenta de usuario para configurar las llamadas de emergencia. 

- Si el cliente de Teams no puede obtener una TeamsEmergencyCallRoutingPolicy, entonces el usuario no está habilitado para hacer llamadas de emergencia.
...
Use el TeamsEmergencyCallingPolicy para configurar quién debe recibir una notificación durante una llamada de emergencia y cómo se la notificará: solo chat, en conferencia, en conferencia y silenciado, o en conferencia y silenciado, pero con la capacidad de reactivar el audio.  También puede especificar un número de RTC externo de un usuario o un grupo al que llamar y unirse a la llamada de emergencia. 

Se puede conceder una TeamsEmergencyCallingPolicy a una cuenta de usuario de Teams, a un sitio de red o a ambas.  Cuando un cliente de equipo inicia o cambia una conexión de red, Teams realiza una búsqueda del sitio de red donde se encuentra el cliente:

- Si un TeamsEmergencyCallingPolicy está asociado con un sitio de red, se usa la Directiva del sitio para configurar la notificación del servicio de seguridad.

- Si no hay ningún TeamsEmergencyCallingPolicy asociado al sitio, o si el cliente está conectado a un sitio no definido, la TeamsEmergencyCallingPolicy asociada a la cuenta de usuario se usa para configurar la notificación del servicio de seguridad.  

- Si el cliente de Teams no puede obtener una TeamsEmergencyCallingPolicy, entonces el usuario no está habilitado para recibir notificaciones sobre el servicio de seguridad.

Durante una llamada de emergencia, un servicio de seguridad se Conferencia en la llamada y la experiencia del usuario del escritorio se controla en función de la TeamsEmergencyCallingPolicy. Una conversación grupal se inicia con cada miembro del centro de seguridad y la ubicación de la persona que llama de emergencia se comparte a través de una notificación de mensaje importante.  Si una opción de conferencia está configurada como parte de la Directiva, también se llamará a cada usuario del escritorio de seguridad como parte de la Conferencia.
