...
MS.collection:
- M365-voice
description: Obtenga información sobre cómo usar el Teams informe de uso de RTC en el centro de administración de Microsoft Teams para obtener información general sobre el uso de llamadas y audioconferencias en su organización.
appliesto:
- Microsoft Teams
...
1. En el panel de navegación izquierdo del centro de administración de Microsoft Teams, haga clic en **Análisis &** **informesAdministrar** >  informes. En la pestaña **Ver informes** , en **Informe**, seleccione **Informe de uso de RTC**.
...
- Llamar desde un número que no es E.164. Un usuario de UN PBX de terceros interconectado con enrutamiento directo realiza una llamada a un usuario de Teams. En este caso, el número del autor de la llamada podría ser cualquier número que no sea E.164, por ejemplo +1001. 
...
#### <a name="phone-number-obfuscation"></a>Teléfono ofuscación del número
...
1.    Usuario de RTC 1 en un punto de conexión RTC llamado Teams usuario 1 en Teams cliente, tipo de llamada Dr_In, id. de correlación 57f28917-42k5-4c0c-9433-79734873f2ac, sin id. de correlación compartido.
2.    Teams usuario 1 en Teams cliente llamado Usuario RTC 1 en un punto de conexión RTC, tipo de llamada Dr_Out 2c12b8ca-62eb-4c48-b68d-e451f518ff4, sin id. de correlación compartida.
3.    Usuario de RTC 1 en un punto de conexión RTC denominado usuario 2 de Teams en Teams cliente, tipo de llamada Dr_In f45e9a25-9f94-46e7-a457-84f5940efde9, id. de correlación compartido f45e9a25-9f94-46e7-a457-84f5940efde9.
4.    Llamada 3 existente con id. de correlación "f45e9a25-9f94-46e7-a457-84f5940efde9". Usuario de RTC 1 en una llamada con Teams usuario 2. Teams Usuario 2 transfirió (ciego o consultativo) una llamada a Teams o Usuario RTC, tipo de llamada Dr_Out_User_Transfer 45a1da7c-9e97-481a-8a05-3fe19a9a77e0, id. de correlación compartido f45e9a25-9f94-46e7-a457-84f5940efde9.
...
> | 5 | Hora de invitación | `datetimeoffset` | Cuando la invitación inicial se envía de salida desde Teams llamada de usuario o bot al SBC, o se recibe en una llamada entrante a Teams o bot por el componente de proxy SIP del enrutamiento directo desde el SBC |
> | 6 | Hora de inicio | `datetimeoffset` | Hora en que el proxy SIP recibió la respuesta final (mensaje SIP "200 OK") desde el SBC en saliente (Teams/Bot a un usuario RTC), o después de que el proxy SIP envíe la invitación al próximo salto dentro de Teams back-end en la llamada entrante (usuario RTC a un Teams/Bot).<br/>Para las llamadas con errores y sin responder, esto puede ser igual a tiempo de invitación o de error |
...
> | 11 | Número de autor de la llamada | `nvarchar(32)` | Número del usuario o bot que realizó la llamada. En la entrada a una llamada de usuario de equipo será un usuario de RTC, en la salida desde Teams llamada de usuario será el número de usuario Teams |
> | 12 | Número de destinatario | `nvarchar(32)` | Número del usuario o bot que recibió la llamada. En la entrada a una llamada de usuario de equipo será el usuario de Teams, de salida desde Teams llamada del usuario será el usuario de RTC |
