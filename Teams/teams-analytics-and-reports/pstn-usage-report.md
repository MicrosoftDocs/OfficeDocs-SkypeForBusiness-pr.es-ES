...
Estos campos pueden proceden del SBC del cliente. Hay tres formatos que el SBC puede enviar a enrutamiento directo: números E.164, números que no sean E.164 y cadenas.
...
1.    Usuario DE RTC 1 en un punto de conexión de RTC llamado Usuario de Teams 1 en el cliente de Teams, tipo de llamada Dr_In, id. de correlación 57f28917-42k5-4c0c-9433-79734873f2ac, ningún id. de correlación compartida.
...
> | 2 | Id. de conferencia | `nvarchar(64)` | Identificador de la audioconferencia |
...
> | 8 | Tipo de llamada | `nvarchar(32)` | Si la llamada era una llamada rtc de salida o entrante y el tipo de llamada, como una llamada realizada por un usuario o una audioconferencia |
...
Puede exportar datos hasta cinco meses (150 días) a partir de la fecha actual, a menos que las normativas específicas del país prohíban la conservación de los datos durante ese período.
...
> | 5 | Hora de inicio | `datetimeoffset` | Hora en la que el proxy SIP ha recibido la respuesta final (mensaje SIP "200 Ok") del SBC en el saliente (Equipos/Bot para un usuario RTC), o después de que el proxy SIP envíe la invitación al próximo salto dentro del back-end de Teams en las llamadas entrantes (Usuario de RTC a un equipo/bot).<br/>Para llamadas con errores y sin responder, puede ser igual a invitación o tiempo de error |
> | 6 | Tiempo de error | `datetimeoffset` | Solo existe para llamadas con errores (no establecidos completamente) |
> | 7 | Hora de finalización | `datetimeoffset` | Solo existe para llamadas correctas (establecidas completamente). Hora en la que la llamada ha finalizado |
> | 8 | Duración (segundos) | `int` | Duración de la llamada |
> | 9 | Correcto | `nvarchar(3)` | Sí/No. Éxito o intento |
> | 10 | Número de autor de la llamada | `nvarchar(32)` | Número del usuario o bot que realizó la llamada. Al llamar a un usuario de equipo, será un usuario de RTC, al salir de la llamada de usuario de Teams, será el número de usuario de Teams. |
