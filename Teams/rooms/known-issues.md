...
|  El uso compartido de contenido de reuniones sfb no muestra pantalla completa         |    En Skype Empresarial reuniones, las salas con pantallas frontales que usan una configuración de PPP alta pueden experimentar problemas en los que el contenido compartido en una reunión no muestra la pantalla completa en la parte frontal de la pantalla de la sala. Esto se debe a un problema subyacente en la API Windows 10 protocolo de escritorio remoto (RDP). | Use la `<WinRTRdpEnabled>` configuración XML para deshabilitar la API Windows 10 RDP para resolver este problema. Para deshabilitarlo, debe especificar el valor como `false` . Para obtener más información, vea [Administrar la configuración de la consola con un archivo de configuración XML.](xml-config-file.md#manage-console-settings-with-an-xml-configuration-file) | Ninguna |
