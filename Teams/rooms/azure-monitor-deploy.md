...
7. [Configurar :::no-loc text="Azure Monitor"::: soluciones adicionales](azure-monitor-deploy.md#Solutions)
...
Debe tener un área de :::no-loc text="Log Analytics"::: trabajo para iniciar la recopilación de registros de los :::no-loc text="Microsoft Teams Rooms"::: dispositivos. Un área de trabajo es un :::no-loc text="Log Analytics"::: entorno único con su propio repositorio de datos, orígenes de datos y soluciones. Si ya tiene un área de :::no-loc text="Log Analytics"::: trabajo existente, puede usarla para supervisar la :::no-loc text="Microsoft Teams Rooms"::: implementación o, de forma alternativa, puede crear un :::no-loc text="Log Analytics"::: área de trabajo dedicada específica para sus :::no-loc text="Microsoft Teams Rooms"::: necesidades de supervisión.

Si necesita crear un :::no-loc text="Log Analytics"::: área de trabajo nueva, siga las instrucciones del artículo [crear un :::no-loc text="Log Analytics"::: área de trabajo en el :::no-loc text="Azure"::: portal](https://docs.microsoft.com/azure/azure-monitor/learn/quick-create-workspace)

> [!NOTE]
> Para usar :::no-loc text="Log Analytics"::: con :::no-loc text="Azure Monitor"::: , necesita tener una :::no-loc text="Azure"::: suscripción activa. Si no tiene una :::no-loc text="Azure"::: suscripción, puede crear [una suscripción de prueba gratuita](https://azure.microsoft.com/free) como punto de partida.
...
Debe configurar :::no-loc text="Log Analytics"::: para recopilar los registros necesarios para supervisar el :::no-loc text="Microsoft Teams Rooms"::: Estado del dispositivo y de la aplicación. :::no-loc text="Microsoft Teams Rooms"::: los dispositivos usan el **:::no-loc text="Skype Room System":::** registro de eventos.

Para configurar :::no-loc text="Log Analytics"::: para recopilar los :::no-loc text="Microsoft Teams Rooms"::: eventos, consulte [ :::no-loc text="Windows"::: orígenes de datos del :::no-loc text="Azure Monitor"::: registro de eventos en](https://docs.microsoft.com/azure/azure-monitor/platform/data-sources-windows-events)
...
Después de :::no-loc text="Microsoft Monitoring"::: que el agente se haya implementado en los dispositivos de prueba, compruebe que los datos del registro de eventos necesarios sean recopilados por :::no-loc text="Azure Monitor"::: .
...
4.  Genere un problema de hardware y compruebe que los eventos necesarios estén registrados :::no-loc text="Azure Log Analytics"::: .
    ...
    2.  Espere 10 minutos para que se rellene el registro de eventos :::no-loc text="Azure Log Analytics"::: .
...
5.  Genere un problema de aplicación y compruebe que los eventos necesarios estén registrados.
    1.  Modifique la :::no-loc text="Microsoft Teams Rooms"::: configuración de la aplicación y escriba una pareja incorrecta de dirección y contraseña del Protocolo de inicio de sesión (SIP).
    2.  Espere 10 minutos para que se rellene el registro de eventos :::no-loc text="Azure Log Analytics"::: .
...
> 
> Preste atención a las consultas necesarias para cada campo personalizado de la tabla siguiente. Debe usar las consultas correctas para :::no-loc text="Log Analytics"::: poder extraer correctamente los valores de los campos personalizados.
> 
**Tabla 1**
...
-   Iniciando un runbook en el :::no-loc text="Azure Automation"::: servicio

Consulte [registrar alertas en :::no-loc text="Azure Monitor"::: ](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-unified-log) para obtener más información sobre las alertas de :::no-loc text="Azure Monitor"::: .

> [!NOTE]
> En los siguientes ejemplos se envían alertas por correo electrónico cuando un :::no-loc text="Microsoft Teams Rooms"::: dispositivo genera un error de hardware o de aplicación.

### <a name="configure-an-email-alert-for-no-loc-textmicrosoft-teams-rooms-hardware-issues"></a>Configurar una alerta por correo electrónico para :::no-loc text="Microsoft Teams Rooms"::: problemas de hardware

Configure una regla de alerta que busque :::no-loc text="Microsoft Teams Rooms"::: dispositivos que hayan encontrado problemas de hardware dentro de la última hora.
1.  Inicie sesión en el [ :::no-loc text="Microsoft Azure"::: portal](https://portal.azure.com) y vaya a :::no-loc text="Log Analytics"::: su área de trabajo y selecciónela.
...
### <a name="configure-an-email-alert-for-no-loc-textmicrosoft-teams-rooms-application-issues"></a>Configurar una alerta por correo electrónico para los problemas de las :::no-loc text="Microsoft Teams Rooms"::: aplicaciones
...
### <a name="deploying-no-loc-textmicrosoft-monitoring-agent-by-using-a-group-policy-object-gpo"></a>Implementación :::no-loc text="Microsoft Monitoring"::: del agente mediante un objeto de directiva de grupo (GPO)

Si ya ha implementado los :::no-loc text="Microsoft Teams Rooms"::: dispositivos antes de implementarlos :::no-loc text="Azure Monitoring"::: , puede usar el script proporcionado para configurar y configurar los agentes mediante :::no-loc text="Active Directory"::: objetos de directiva de grupo.

1.  Crear una ruta de red compartida y conceder acceso de lectura al grupo **equipos del dominio** .

2.  Descargar la versión de 64 bits del :::no-loc text="Microsoft Monitoring"::: agente para :::no-loc text="Windows"::: de <https://go.microsoft.com/fwlink/?LinkID=517476>

3.  Extraiga el contenido del paquete de instalación en el recurso compartido de red.
    1.  Abra una ventana del símbolo del sistema y, a continuación, ejecute **MMASetup-AMD64.exe/c**
    2.  Especifique el recurso compartido que acaba de crear y extraiga el contenido.

4.  Cree un nuevo objeto de directiva de grupo y asígnelo a la unidad organizativa donde :::no-loc text="Microsoft Teams Rooms"::: se encuentren las cuentas de equipo.

5.  Configurar la Directiva de ejecución de PowerShell:
    1.  Modificar el objeto de directiva de grupo recién creado y navegar a directivas de configuración del equipo \\ \\ \\ :::no-loc text="Windows"::: componentes \\ de plantillas administrativas :::no-loc text="Windows PowerShell":::
