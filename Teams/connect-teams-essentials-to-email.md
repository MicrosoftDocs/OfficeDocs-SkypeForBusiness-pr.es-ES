...
2. En el menú de navegación izquierdo, vaya **a Correo &** colaboraciónPolicias  > **& reglas** >  >  Desatencionar **directivasAnti-spam** en la sección Directivas
...
Para que el acceso al calendario funcione para los buzones locales, siga las instrucciones que se proporcionan en [Teams Configuración](https://techcommunity.microsoft.com/t5/exchange-team-blog/configuring-teams-calendar-access-for-exchange-on-premises/ba-p/1484009) de un acceso de calendario para Exchange buzones locales: Microsoft Tech Community
...
Estos ejemplos se basan en [el Conectar de PowerShell de ExchangeOnline](/powershell/module/exchange/connect-exchangeonline?view=exchange-ps&preserve-view=true) que forma parte del módulo Exchange Online [PowerShell V2](/powershell/exchange/exchange-online-powershell-v2&preserve-view=true). Si recibe un error al ejecutar Conectar-ExchangeOnline, asegúrese de que ha seguido las instrucciones recomendadas para instalar el módulo con [Instalar el módulo EXO V2](/powershell/exchange/exchange-online-powershell-v2?view=exchange-ps&preserve-view=true). Cuando Connect-ExchangeOnline solicita credenciales, asegúrese de usar una cuenta de administrador de inquilinos.
...
2. Vaya **a ConfigurarDominios** >  y seleccione **Agregar dominio** para agregar el dominio existente. Si no agrega un dominio, los usuarios de su organización usarán el dominio onmicrosoft.com para sus direcciones de correo electrónico hasta que lo haga. Asegúrese de agregar el dominio antes de agregar usuarios, para que no tenga que configurarlos dos veces.
...
8. Vuelva a iniciar sesión en Centro de administración de Microsoft 365 en <https://admin.microsoft.com/AdminPortal/> para habilitar DKIM

9. En el panel de navegación de la izquierda, seleccione **SetupDomains**  > 

10. Con la casilla, seleccione su dominio que no es de Microsoft (por ejemplo, TomislavK@thephone-company.com) de las listas de dominios actuales.
...
1. Ir al centro de administración en para <https://admin.microsoft.com> agregar un usuario individual

2. Vaya a **UsuariosActivos** >  y seleccione **Agregar un usuario**
...
15. En el **campo Si coinciden las expresiones anteriores,** haga lo siguiente > **Modificar** mensaje y active Omitir filtro de **correo** no deseado para este mensaje en **Correo no deseado**.
...
4. En el **Exchange** de navegación izquierdo del centro de administración,  >  seleccione Flujo de **correoConectores**, abra el menú desbordamiento (...) y seleccione Agregar un conector
...
11. En la **sección Correo** electrónico de validación, escriba una dirección de correo electrónico válida para el sistema gmail (por ejemplo, johannal@g.contosolandscaping2.m365master.com), seleccione el signo más **(+)** y, a continuación, seleccione **Validar**
...
1. Use el **Administración de Microsoft 365 de** correo electrónico para actualizar cada buzón o puede usar un script de **PowerShell**, como el siguiente:
    ...
    **Solución de problemas Conectar-ExchangeOnline:**
    ...
    ¿Experimenta un error al ejecutar Conectar-ExchangeOnline? Este podría ser el resultado de la regla de reenvío automático de correo electrónico de su organización. De forma predeterminada, el reenvío automático está deshabilitado. Para conectar Teams Essentials a Google Workspace, la regla debe estar habilitada.  
...
1. Siga las instrucciones de [Habilitar o deshabilitar Outlook en la Web](/exchange/recipients-in-exchange-online/manage-user-mailboxes/enable-or-disable-outlook-web-app) para un buzón en Exchange Online para deshabilitar Outlook en la Web buzones.
