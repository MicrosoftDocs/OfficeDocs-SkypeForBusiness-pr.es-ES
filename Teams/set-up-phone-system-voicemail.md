...
El aprovisionamiento para usuarios de Teams no es lo mismo que para los usuarios de Skype Empresarial En línea. Para los usuarios de Skype Empresarial Online, Correo de voz en la nube se configuró y aprovisionó automáticamente cuando se asignó a los usuarios una licencia de Sistema telefónico y el sistema de aprovisionamiento Telefonía IP empresarial habilitado.
...
Los mensajes de correo de voz generados por Correo de voz en la nube se entregan y se almacenan en el buzón Exchange del usuario, ya sea en Exchange Online o en Exchange Server. No hay almacenamiento específico o adicional para el correo de voz. Los clientes que tienen acceso al correo de voz (por ejemplo, Microsoft Outlook, Microsoft Teams o Skype Empresarial) lo hacen a través del buzón de Exchange y de las API proporcionadas.
...
La grabación y transcripción de Correo de voz en la nube comienza en Microsoft 365 en el origen de la llamada que se dirige a Correo de voz en la nube. Después, el mensaje se entrega al buzón de correo Exchange del usuario.

Por ejemplo, si una llamada llega a un usuario de enrutamiento directo no disponible a través de un controlador de borde de sesión (SBC) en Europa, la grabación y la transcripción del correo de voz se realizan en Europa. Después, el mensaje se entrega al buzón de correo Exchange del usuario. Por otro ejemplo, suponga que un usuario Teams en Norteamérica llama a un usuario Teams no disponible en Europa. En este caso, la llamada se inicia en Norteamérica, el procesamiento se produce en Norteamérica y, a continuación, el correo de voz se entrega al buzón de correo Exchange del usuario en Europa.
...
Para administrar las características de Correo de voz en la nube para los usuarios, use el módulo Teams PowerShell como se indica a continuación.
...
1. Los mensajes de correo de voz se entregan al buzón de Exchange de un usuario a través de SMTP enrutados a través de Exchange Online Protection. Para habilitar la entrega correcta de estos mensajes, asegúrese de que los conectores de Exchange estén correctamente configurados entre los servidores de Exchange y Exchange Online Protection. Para obtener más información, vea [Usar conectores para configurar Flow de correo](/exchange/mail-flow-best-practices/use-connectors-to-configure-mail-flow/use-connectors-to-configure-mail-flow).

2. Para habilitar las características del correo de voz, como la personalización de saludos y el correo de voz visual en Teams clientes, debe configurar la conectividad entre Microsoft 365 y el buzón de Exchange Server. Para habilitar esta conectividad, debe configurar el nuevo protocolo de autenticación de Oauth Exchange que se describe en [Configurar la autenticación de OAuth entre Exchange y Exchange Online organizaciones](/exchange/configure-oauth-authentication-between-exchange-and-exchange-online-organizations-exchange-2013-help), o ejecutar el Asistente para Exchange híbrido desde Exchange 2013 CU5 o superior. También debe configurar la integración y Oauth entre Teams y Exchange Server que se describen en [Configurar integración y OAuth entre Teams y Exchange Server](/skypeforbusiness/deploy/integrate-with-exchange-server/oauth-with-online-and-on-premises).
