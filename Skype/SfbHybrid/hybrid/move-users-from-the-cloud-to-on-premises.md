...
- La organización debe tener Azure AD Conectar configurado correctamente y estar sincronizando todos los atributos relevantes para el usuario, como se describe en [Configure Azure AD Conectar](configure-azure-ad-connect.md).
...
- El usuario interactúa con la implementación Skype Empresarial Server para su funcionalidad. 
- Los contactos que existían en Teams se migran a Skype Empresarial Server. Los dos conjuntos de contactos se combinan y, a continuación, se migran de nuevo a local.  Además, los contactos que ya existen en Teams permanecen en Teams.
...
- Si no tiene una cuenta con permisos suficientes tanto en el servicio local como en el servicio en la nube (Microsoft 365), use el parámetro -credential para proporcionar a una cuenta permisos suficientes en Microsoft 365.
- Si la cuenta con permisos en Microsoft 365 no termina en "on.microsoft.com", debe especificar el parámetro -HostedMigrationOverrideUrl, con el valor correcto como se describe en [Credenciales](move-users-between-on-premises-and-cloud.md#required-administrative-credentials)administrativas necesarias .

La siguiente secuencia de cmdlets se puede usar para mover un usuario a Skype Empresarial Server y se supone que la credencial Microsoft 365 es una cuenta independiente y se proporciona como entrada para el Get-Credential solicitud.
...
### <a name="move-users-with-the-skype-for-business-server-control-panel"></a>Mover usuarios con el Panel Skype Empresarial Server control
...
Para quitar la asignación del usuario de TeamsUpgradePolicy, ejecute el siguiente cmdlet desde una ventana Teams PowerShell:
