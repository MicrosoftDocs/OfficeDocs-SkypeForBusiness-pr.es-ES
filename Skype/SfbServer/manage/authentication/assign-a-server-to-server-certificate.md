...
Para determinar si un certificado de autenticación de servidor a servidor ya se ha asignado a Skype empresarial Server, ejecute el siguiente comando desde el shell de administración de Skype empresarial Server:
  
```PowerShell
Get-CsCertificate -Type OAuthTokenIssuer
```

Si no recibe información del certificado, necesita asignar un certificado del emisor de token para poder usar la autenticación de servidor a servidor. Como regla general, cualquier certificado de Skype empresarial Server puede usarse como su certificado OAuthTokenIssuer. por ejemplo, el certificado predeterminado de Skype empresarial Server también se puede usar como certificado OAuthTokenIssuer. (El certificado OAUthTokenIssuer también puede ser cualquier certificado de servidor Web que incluya el nombre de su dominio SIP en el campo asunto). Los dos requisitos principales para el certificado que se usa para la autenticación de servidor a servidor son los siguientes: 1) el mismo certificado se debe configurar como el certificado de OAuthTokenIssuer en todos los servidores front-end; y 2) el certificado debe tener al menos 2048 bits.
...
En el comando anterior, el certificado recuperado está configurado para funcionar como certificado de autenticación de servidor a servidor global; eso significa que el certificado será replicado y usado por todos los servidores front-end. De nuevo, este comando solo se debe ejecutar una vez y solo en uno de los servidores front-end. Aunque todos los servidores front-end deben usar el mismo certificado, no debe configurar el certificado OAuthTokenIssuer en cada servidor front-end. En su lugar, configure el certificado una vez y deje que el servidor de replicación de Skype empresarial Server se ponga al lado de copiar el certificado en cada servidor.
  
El cmdlet Set-CsCertificate toma el certificado en cuestión y lo configura inmediatamente para que actúe como el certificado de OAuthTokenIssuer actual. (Skype empresarial Server conserva dos copias de un tipo de certificado: el certificado actual y el certificado anterior). Si necesita que el nuevo certificado empiece a funcionar inmediatamente como el certificado OAuthTokenIssuer, debe usar el cmdlet Set-CsCertificate.
...
El 1 de julio de 2015, el nuevo certificado se configurará como el certificado de OAuthTokenIssuer actual y el certificado de OAuthTokenIssuer "antiguo" se configurará como el certificado anterior.
...
> [!CAUTION]
> En este caso, el procedimiento debe realizarse en cada servidor front-end. Al exportar e importar certificados de esta forma, Skype empresarial Server no replicará el certificado en cada servidor front-end. 
...
3. En la página de Skype empresarial Server, haga clic en el botón **Ejecutar** debajo del título **paso 3: solicitar, instalar o asignar certificados**. Nota: si ya ha instalado certificados en este equipo, en lugar del botón **Ejecutar**, aparecerá **Ejecutar de nuevo**.
