---
title: Iniciar servicios
ms.author: jambirk
author: jambirk
manager: serdars
ms.audience: ITPro
ms.topic: article
f1_keywords:
- ms.lync.dep.DeployMainStartServices
ms.prod: skype-for-business-itpro
localization_priority: Normal
ms.assetid: efaf7b2a-257f-4aef-9c25-40ea93b03ef4
ROBOTS: NOINDEX, NOFOLLOW
description: 'Para iniciar los servicios en el rol de servidor implementado, haga clic en ejecutar en el paso 4: iniciar servicios. Debe iniciar sesión como miembro del grupo local Administradores o equivalente.'
ms.openlocfilehash: 1c2d76bd0dffa9541dce8e66883d54be8db74031
ms.sourcegitcommit: e9f277dc96265a193c6298c3556ef16ff640071d
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 07/24/2018
ms.locfileid: "20981596"
---
# <a name="start-services"></a>Iniciar servicios
 
Para iniciar los servicios en el rol de servidor implementado, haga clic en **Ejecutar** en **paso 4: iniciar servicios**. Debe iniciar sesión como miembro del grupo local Administradores o equivalente. 
  
> [!NOTE]
> Si se ha ejecutado **Iniciar servicios** , puede volver a ejecutar el comando haciendo clic en **Ejecutar de nuevo**. 
  

