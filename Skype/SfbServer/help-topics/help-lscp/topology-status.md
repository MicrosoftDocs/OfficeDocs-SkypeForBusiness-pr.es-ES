---
title: Estado de topología
ms.reviewer: ''
ms.author: v-cichur
author: cichur
manager: serdars
ms.date: 5/21/2015
audience: ITPro
ms.topic: article
f1.keywords:
- CSH
ms.custom:
- ms.lync.lscp.TopoStatusMain
ms.prod: skype-for-business-itpro
ms.localizationpriority: medium
ms.assetid: d5f858f5-df8e-43a9-80aa-6ba1ddb27459
description: 'La página Topología: Estado muestra el estado de los servidores de la Skype Empresarial topología.'
ms.openlocfilehash: 4b500cf9f8144dac68ec0a01944f09b8c4555727
ms.sourcegitcommit: 556fffc96729150efcc04cd5d6069c402012421e
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 08/26/2021
ms.locfileid: "58605179"
---
# <a name="topology-status"></a>Topología: Estado

La **página Topología**: **Estado** muestra el estado de los servidores de la Skype Empresarial topología.

## <a name="tasks-you-can-perform"></a>Tareas que puede realizar

Puede realizar las siguientes tareas en la página **Topología**: **Estado**:

- [Ver una lista de equipos que ejecutan Lync Server 2010](/previous-versions/office/lync-server-2013/lync-server-2013-view-a-list-of-computers-running-lync-server-2013)

- [Ver detalles sobre un servicio](/previous-versions/office/lync-server-2013/lync-server-2013-view-details-about-a-service)

- [Iniciar o detener servicios de Lync Server 2010](/previous-versions/office/lync-server-2013/lync-server-2013-start-or-stop-lync-server-services)

- [Impedir nuevas conexiones a servicios](/previous-versions/office/lync-server-2013/lync-server-2013-prevent-sessions-for-services)

Para obtener información detallada sobre los diferentes procedimientos que puede realizar mediante el Panel de control de Skype Empresarial Server, vea [Manage Skype Empresarial Server 2015](../../manage/manage.md).