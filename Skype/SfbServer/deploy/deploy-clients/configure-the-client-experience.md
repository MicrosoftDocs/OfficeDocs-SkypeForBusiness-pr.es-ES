...
Puede especificar la experiencia del cliente que verán los usuarios de su organización mediante el cmdlet **Set-CSClientPolicy** con el parámetro EnableSkypeUI:
...
El siguiente comando selecciona la experiencia de cliente de Lync para todos los usuarios de la organización afectados por la directiva global:
...
1. Confirme que el valor de  `EnableSkypeUI` está establecido en $False en la directiva que está usando como se describió anteriormente.
...
2. Actualice el Registro del sistema en el equipo del usuario. Debe hacerlo antes de que la primera vez que los usuarios inicien el Skype Empresarial cliente y solo debe hacerlo una vez. Para obtener información sobre cómo crear un objeto de directiva de grupo para actualizar el Registro en un equipo unido a un dominio, vea la sección más adelante en el tema.
    ...
    El **nombre del valor** debe **ser EnableSkypeUI** y los datos **value** deben establecerse en **00 00 00 00**.
...
Si no desea que los usuarios puedan acceder al tutorial, puede desactivar el tutorial de cliente con el siguiente valor del Registro:
  
En la **clave [HKEY_CURRENT_USER\Software\Microsoft\Office\15.0\Lync]** , cree un nuevo **valor DWORD (32 bits**). El **nombre del valor** debe ser **TutorialFeatureEnabled** y los **datos value** deben establecerse en **0**.
...
|:-----|:-----|:-----|
|Skype Empresarial Server |Predeterminada  <br/> |Skype Empresarial  <br/> |
...
1. Inicie la consola **de administración de directivas de grupo**.
    ...
    Para obtener información sobre cómo usar la Consola de administración de directivas de grupo, vea [Group Policy Management Console](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/dn265969(v=ws.11)).
...
5. En el **Editor de administración de directivas de** grupo, expanda **Configuración** de usuario, Preferencias **, expanda** **Windows Configuración y,** a continuación, seleccione el **nodo** Registro.
    
6. Haga clic con el botón secundario en **el nodo Registro** y, a continuación, **seleccione Nuevo** >  **Elementoregistry**.
...
A continuación, deberá vincular el GPO que creó al grupo de usuarios a los que desea asignar la directiva, como una OU.
  
### <a name="to-use-the-gpo-to-assign-the-policy"></a>Para usar el GPO para asignar la directiva

1. En la Consola de administración de directivas de grupo, haga clic con el botón secundario en la OU a la que desea asignar la directiva y, a continuación, **seleccione Vincular a un GPO existente**.
...
También puede comprobar que el GPO ha actualizado correctamente el Registro en el equipo de un usuario examinando el Registro. Abra el Editor del Registro y vaya a **la tecla [HKEY_CURRENT_USER\Software\Microsoft\Office\Lync** ]. Si el GPO actualizó correctamente el Registro, verá un valor denominado EnableSkypeUI con un valor de 0.
