...
|:-----|:-----|:-----|
|storedUserName  <br/> |Sí  <br/> |Cuenta de usuario (en el formato dominio\nombredeusuario) utilizada para obtener acceso al almacén de supervisión; por ejemplo:  <br/> ```-storedUserName "litwareinc\kenmyer"```Esta cuenta debe tener los permisos de SQL Server y SQL Server Reporting Services especificados anteriormente o el script producirá un error.  <br/> |
...
|reportSqlServerInstance  <br/> |No  <br/> |La instancia de SQL Server en que se hospeda el servicio de informes. La instancia se debe especificar usando el nombre de dominio completo del servidor de informes; por ejemplo:<br/> ```-reportServerSqlInstance atl-sql-001.litwareinc.com```Si no se incluye este parámetro, el script asumirá que los servicios de informes están hospedados por la misma instancia SQL Server que hospeda la base de datos de supervisión.  <br/> |
...
Después de instalar los informes de supervisión, debe usar el cmdlet New-CsReportingConfiguration para configurar la dirección URL usada para tener acceso a estos informes. Esta tarea se puede llevar a cabo desde el Shell Skype Empresarial Server administración mediante la ejecución del siguiente Windows PowerShell comandos. Tenga en cuenta que se recomienda, aunque no es obligatorio, utilizar el protocolo HTTPS en la configuración de la dirección URL de informes:
