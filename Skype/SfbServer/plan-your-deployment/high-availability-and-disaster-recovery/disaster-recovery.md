...
Tener dos centros de datos en todas las regiones del mundo es posible, pero podría suponer la mayor pérdida de datos si hay un desastre, debido a la latencia de replicación de datos.
  
Cuando planee qué grupos a par, debe tener en cuenta que sólo los pares siguientes son compatibles:
...
## <a name="backup-registrar-relationships-and-survivable-branch-appliances"></a>Copia de seguridad relaciones de registrador y aparatos de sucursal que sobreviven

Además de proporcionar la capacidad de recuperación ante desastres, dos grupos de servidores emparejados sirven como registradores de copia de seguridad entre sí. Cada grupo puede ser la copia de seguridad para otro sólo un grupo de Front-End.
  
Aunque copia de seguridad relaciones entre dos grupos de servidores de Front-End deben ser simétrica y de 1:1, cada grupo de servidores Front-End aún también puede ser el registrador de copia de seguridad para cualquier número de dispositivos de sucursales que sobreviven.
...
Para el grupo de conmutación por error y conmutación por recuperación del grupo, el destino de ingeniería para objetivo de punto de recuperación (RPO) es de 5 minutos. Esto representa la medida de tiempo de los datos pueden perderse debido al desastre, debido a la latencia de replicación del servicio de copia de seguridad. Por ejemplo, si un grupo deja de funcionar a las 10:00 A.M. y el RPO es de 5 minutos, los datos escritos en el grupo entre las 9:55 AM. y .might de 10:00 A.M. no se han replicado en el grupo de copia de seguridad y se perdería.
...
El almacén de administración central contiene datos de configuración sobre los servidores y los servicios de tu implementación. Cada Skype para la implementación de Business Server incluye un almacén de Administración Central, que aloja el servidor de fondo detrás de un grupo de servidores Front-End.
...
Durante un failover de grupo que implica la agrupación que aloja el almacén de Administración Central, debe conmutar el almacén de Administración Central antes de conmutar el grupo de servidores Front-End.
...
Los destinos de ingeniería para failover del almacén de Administración Central son 5 minutos para el objetivo de tiempo de recuperación (RTO) y 5 minutos para la recuperación de punto de recuperación (RPO).
...
Cualquier empresa que implementa Skype para Business Server a través de varios centros de datos y utiliza la característica de recuperación de desastres debe asegurarse de que el tráfico entre los centros de datos está protegido por la Intranet corporativa. Las empresas que tienen en cuenta la protección frente a ataques internos necesitan proteger los vínculos de comunicación entre los centros de datos. Este es un requisito estándar que también ayuda a proteger muchos más tipos de datos confidenciales corporativos que se transfieren entre los centros de datos.
...
Posiblemente cuentes con varias maneras de mejorar la protección de seguridad para el tráfico del servicio de copia de seguridad. Estas maneras varían desde restringir el acceso a los centros de datos hasta proteger el transporte de WAN entre los dos centros de datos. En la mayoría de los casos, las empresas que distribuyen Skype para Business Server que ya tenga la infraestructura de seguridad requerida en el lugar. Para aquellas empresas que necesitan una guía, Microsoft brinda una solución como un ejemplo de cómo desarrollar una infraestructura de TI segura. Para obtener más información, consulte [https://go.microsoft.com/fwlink/p/?LinkId=268544](https://go.microsoft.com/fwlink/p/?LinkId=268544). 
  
Nosotros no implican que es la única solución, ni que se supone que es la solución preferida para Skype para Business Server. Recomendamos que los clientes empresariales elijan la solución que se ajuste a sus necesidades específicas, de acuerdo con los requisitos y la infraestructura de TI que posean. El ejemplo de Microsoft emplea IPSec y directivas de grupo para el servidor y el aislamiento de dominios.
...
- El servicio SMB (TCP/445) de cada servidor Front-End de grupo de servidores al almacén de archivos utilizados por el grupo de servidores B.
    
- El servicio SMB (TCP/445) de cada servidor Front-End en el grupo B al almacén de archivos utilizada por el grupo A.
    
> [!CAUTION]
>  IPsec no está pensado para reemplazar la seguridad de la aplicación, como SSL/TLS. Una ventaja de utilizar IPsec es que puede ofrecer seguridad de tráfico de red para las aplicaciones existentes sin necesidad de cambiarlas. Las empresas que desean proteger sólo el transporte entre los dos data centers deben consultar sus respectivos proveedores de hardware de red sobre formas de configurar las conexiones WAN seguras con los equipos del proveedor.
  
## <a name="see-also"></a>Vea también

#### 

[Implementar grupos emparejados de Front-End para recuperación ante desastres en Skype para Business Server 2015](../../deploy/deploy-high-availability-and-disaster-recovery/front-end-pools-for-disaster-recovery.md)
