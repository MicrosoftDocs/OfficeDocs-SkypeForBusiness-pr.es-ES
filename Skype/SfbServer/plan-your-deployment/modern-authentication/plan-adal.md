...
- Hay un recurso al que el cliente necesita un nivel de acceso específico y este recurso está protegido por un servicio de directorio, en este caso el recurso es Skype Empresarial Server.
...
En las conversaciones de Autenticación moderna (ADAL) de Skype Empresarial Server, Skype Empresarial Server se comunica a través de ADFS (ADFS 3.0 en Windows Server 2012 R2). Es posible que la autenticación se haga con otro proveedor de identidades (IdP), pero el servidor de Skype Empresarial debe configurarse para comunicarse directamente con ADFS. Si no ha configurado ADFS para que funcione con Skype Empresarial Server, complete la [instalación de ADFS](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/dd727938(v=ws.10)).
...
> [!NOTE]
> Durante la versión inicial, la autenticación moderna en un entorno local solo se admite si no hay una topología mixta de Skype implicada. Por ejemplo, si el entorno es puramente Skype Empresarial Server. Esta instrucción puede estar sujeta a cambios. 
  
Un paquete de PowerShell que incluya archivos .ps1 con los comandos usados por ADAL debe descargarse para una configuración correcta.
