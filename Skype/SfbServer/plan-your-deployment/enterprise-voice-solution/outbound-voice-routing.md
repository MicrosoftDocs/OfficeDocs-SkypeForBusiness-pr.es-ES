...
description: Obtenga información acerca del enrutamiento de voz saliente en Skype para Telefonía IP empresarial de Business Server, incluida la configuración de enrutamiento de llamada, marcar los planes, las reglas de normalización, directivas de voz, registros de uso PSTN y rutas de voz.
...
Obtenga información acerca del enrutamiento de voz saliente en Skype para Telefonía IP empresarial de Business Server, incluida la configuración de enrutamiento de llamada, marcar los planes, las reglas de normalización, directivas de voz, registros de uso PSTN y rutas de voz.
  
Enrutamiento de llamadas salientes se aplica a las llamadas de Telefonía IP empresarial que están destinadas a una puerta de enlace de telefónica pública conmutada (PSTN) de red, tronco o central de conmutación (PBX). Cuando un Skype para usuarios de empresa coloca una llamada, el servidor normaliza el número de teléfono al formato E.164, si es necesario e intenta hacerla coincidir con un URI de SIP. Si el servidor no puede establecer la coincidencia, aplica la lógica de enrutamiento de llamadas salientes basándose en la cadena de marcado proporcionada. Esta lógica la defines con la configuración de las opciones del servidor que se describe en la siguiente tabla.
  
**Skype para Business Server saliente configuración del enrutamiento de llamadas**
...
El alcance de un plan de marcado determina el nivel jerárquico en el que se puede aplicar. En Skype para Business Server, un usuario puede asignar un plan de marcado de cada usuario específico. Si no se asigna un plan de marcado del usuario, se aplica el plan de marcado de grupo de servidores Front-End. Si no hay ningún plan de marcado de grupo de grupo Front-End, se aplica el plan de marcado de sitio. Por último, si no hay otro plan de marcado aplicable al usuario, se aplica el plan de marcado global.
  
Los clientes obtienen niveles de ámbito dial plan mediante la configuración de aprovisionamiento en banda que se proporciona cuando los usuarios inician sesión en Skype para el negocio. Como administrador, puede administrar y asignar niveles de ámbito del plan de marcado mediante Skype para Panel de Control de servidor empresarial.
...
- **Plan de marcado global**: el plan de marcado predeterminado que se instala con el producto. Puedes editar el plan de marcado global, pero no eliminarlo. Este plan de marcado se aplica a todos los usuarios de Telefonía IP empresarial, grupos y objetos de contacto en la implementación, a menos que configura y asigna un plan de marcado con un ámbito más específico.
...
- Decide si son necesarios varios planes de marcado para una sola ubicación. 
    ...
    Si la organización mantiene un único plan de marcación en múltiples ubicaciones, aún necesitará crear un plan de marcación independientes para los usuarios de Telefonía IP empresarial que va a migrar desde una centralita privada (PBX) y que necesitan tener sus extensiones existentes que se conservan.
    
- Decide si son necesarios los planes de marcado por usuario. Por ejemplo, si tiene usuarios en un sitio de sucursal que se registran con el sitio central o si tiene usuarios que están registrados en un dispositivo de la rama que sobreviven, puede considerar especiales escenarios de marcado para dichos usuarios usando por usuario marcar los planes y las reglas de normalización . Para obtener información detallada, consulte [Plan de resiliencia de Telefonía IP empresarial en Skype para Business Server 2015](enterprise-voice-resiliency.md).
    
- Determina el ámbito del plan de marcado (como ya se ha explicado antes en este tema).
    
Para crear un plan de marcado, especifique valores en los siguientes campos, según sea necesario, mediante Skype para Business Server Control Panel o Skype para el Shell de administración de servidor empresarial.
  
#### <a name="name-and-simple-name"></a>Nombre y Nombre simple

En los planes de marcado de usuario necesitas asignar un nombre descriptivo que identifique a los usuarios, grupos u objetos de contacto a los que se asignará el plan de marcado. En el caso de planes de marcado de sitio, el campo Nombre se rellena previamente con el nombre del sitio y no se puede modificar. Para el grupo de planes de marcado, el campo nombre se rellena previamente con la puerta de enlace PSTN o grupo de servidores Front-End completamente (FQDN) del nombre de dominio completo y no se puede cambiar.
  
El plan de marcado de nombre Simple se rellena previamente con una cadena que se deriva el nombre del plan de marcado. El campo Nombre simple se puede editar, y esto le permite crear una convención de denominación más descriptiva para sus planes de marcado. Valor de nombre de TheSimple no puede estar vacío y debe ser único. El procedimiento recomendado es desarrollar una convención de nomenclatura para toda la organización y aplicarla con coherencia en todos los sitios y para todos los usuarios.
...
Puede especificar un prefijo de acceso externo de hasta cuatro caracteres (#, \*y 0-9) si los usuarios necesitan marcar uno o varios líderes dígitos adicionales (por ejemplo, 9) para obtener una línea externa.
...
- Coincide con un número marcado para el URI de SIP del destinatario.
...
Las reglas de normalización usan expresiones regulares de .NET Framework para especificar patrones numéricos coincidentes que el servidor utiliza para convertir las cadenas de marcado a formato E.164 con el fin de realizar búsquedas inversas de números. Crea reglas de normalización en el Skype para Business Server Control Panel introduciendo manualmente las expresiones, o escribiendo la dígitos y la longitud de las cadenas de marcado para coincidir y dejar que el Skype para el Panel de Control de servidor de Business generar la expresión regular correspondiente para usted. Sea como fuere, al finalizar puedes especificar un número de prueba para comprobar que la regla de normalización funciona según lo previsto.
...
**Reglas de normalización de tabla 1 utilizando expresiones regulares de.NET Framework**
...
**La tabla 2. Plan de marcado de Redmond basándose en las reglas de normalización que se muestra en la tabla 1**
...
Skype para directivas de voz Business Server defina lo siguiente para cada usuario, el sitio o la organización a la que se asigna la directiva:
  
- Un conjunto de características que pueden habilitarse o deshabilitarse para determinar la funcionalidad de Telefonía IP empresarial disponible para los usuarios de llamar.
    
- Un conjunto de registros de uso de la red telefónica conmutada (RTC) que definen qué tipo de llamadas están autorizadas. 
    
Los pasos siguientes le ayudará a planear las directivas de voz que necesitará para la implementación de Telefonía IP empresarial:
  
- Determina cómo configurarás la directiva de voz global (la directiva de voz predeterminada que se instala con el producto). Esta directiva se aplicará a todos los usuarios de Telefonía IP empresarial que no se asignan explícitamente una directiva de nivel de sitio o por usuario.
...
Ámbito de la directiva de voz determina el nivel jerárquico en el que se puede aplicar la directiva. En Skype para Business Server, puede configurar directivas de voz con los siguientes niveles de ámbito (enumerados de más específico al más general).
...
- La **directiva de voz de usuario** se puede asignar a usuarios, grupos u objetos de contacto individuales. Es la directiva de nivel más bajo. Las directivas de voz de usuario se pueden implementar para habilitar características para usuarios y grupos determinados en un sitio, pero no para otros en el mismo sitio. Por ejemplo, puedes deshabilitar las llamadas de larga distancia para algunos empleados. Con el fin de asignar una directiva de voz, un objeto de contacto se considera un usuario individual.
    ...
    > [!NOTE]
    > Recomendamos que implemente una directiva de voz del usuario para la rama sitios de Telefonía IP empresarial los usuarios registrados con la implementación del sitio central o los usuarios registrados en un dispositivo de la rama que sobreviven. 
...
- **Malintencionadas llame seguimiento** permite a los usuarios maliciosos llamadas de informe utilizando el Skype para Business client y luego indicadores tales llamadas en los registros de detalle de la llamada. Está deshabilitada de forma predeterminada.
    
- **Escape de correo de voz** evita que las llamadas se enrute inmediatamente al sistema de correo de voz de teléfono móvil del usuario cuando se configura la llamadas simultáneas y el teléfono está desactivado, sin batería o fuera del intervalo y se basa en el valor de un temporizador. Esta configuración habilita y deshabilita el temporizador, y establece su valor. Se puede configurar utilizando el Skype para el Shell de administración de servidor empresarial. Está deshabilitada de forma predeterminada.
    
- **Llame al reenvío y simultánea usos PSTN timbres** permite a los administradores especificar el mismo uso PSTN como la directiva de voz para reenvío de llamadas y llamadas simultáneas, restringir el reenvío de llamadas y las llamadas simultáneas a Skype interno para Los usuarios de negocios, o especificar un uso PSTN personalizadas que difiere del uso de la directiva voz PSTN. La opción predeterminada es usar el mismo uso de RTC que la directiva de voz para el desvío de llamadas y las llamadas simultáneas.
...
Planificar los registros de uso de RTC consiste principalmente en elaborar una lista de todos los permisos de llamada vigentes actualmente en la organización, desde los permisos del presidente hasta los de los empleados temporales, consultores y personal contingente. Este proceso también brinda la oportunidad de volver a examinar los permisos de llamada existentes y modificarlos. Puede crear registros de uso de RTC sólo para esos permisos de llamada que se aplican a los usuarios de Telefonía IP empresarial anticipados, pero podría ser una solución mejor a largo plazo crear registros de uso PSTN para todos los permisos de llamada, independientemente de si algunas pueden no actualmente aplicar al grupo de usuarios esté habilitada para la Telefonía IP empresarial. Si cambian los permisos de llamada o se agregan usuarios nuevos con permisos de llamada diferentes, ya estarán creados los registros de uso de RTC necesarios.
...
Las rutas de llamada especifican cómo Skype para Business Server trata de llamadas salientes realizadas por los usuarios de Telefonía IP empresarial. Cuando un usuario marca un número, el servidor Front-End normaliza la cadena de marcado al formato E.164, si es necesario e intenta hacerla coincidir con un URI de SIP. Si el servidor no puede establecer esa coincidencia, aplicará la lógica de enrutamiento de llamadas salientes basándose en el número. El paso final para definir la lógica consiste en crear una ruta de llamada con nombre independiente para cada conjunto de números de teléfono de destino indicados en cada plan de marcado.
...
Puede especificar las rutas de llamada en el Skype para Business Server Control Panel. Estas llamadas rutas de rellenar la tabla de enrutamiento del servidor, que utiliza Skype para Business Server para dirigir las llamadas que están destinadas a la RTC.
  
### <a name="mn-trunk-support"></a>Compatibilidad con troncos M:N

Skype para Business Server proporciona flexibilidad en cómo se enrutan las llamadas a la RTC. Una ruta de voz especifica a la RTC un conjunto de troncos que pueden usarse para una llamada de voz concreta. Un tronco asocia un servidor de mediación y un número de puerto a una puerta de enlace PSTN y el número de puerto de escucha. Esta asociación lógica permite que un servidor de mediación para asociarse con varias puertas de enlace y tienen varias conexiones a la misma puerta de enlace. Al definir una ruta de llamada, especificar los troncos asociados a esa ruta, pero no especifica que los servidores de mediación están asociados con la ruta. Para crear los troncos mediante la definición de las relaciones entre los servidores de mediación y puertas de enlace PSTN, PBX IP y controladores de borde de sesión (SBCs), utilizan el generador de topología.
...
Para un ejemplo de enrutamiento de menor costo cómo podría utilizar, considere lo siguiente: Fabrikam decide permitir que los usuarios alemanes marcar números de Estados Unidos utilizando el tronco de Estados Unidos. Fabrikam también desea configurar el sistema para que fin todas las llamadas desde los Estados Unidos Skype para los usuarios de Business Server a Alemania y los países o regiones adyacentes del tronco con la puerta de enlace en Alemania. Esta ruta se ahorrar dinero, porque una llamada de Alemania, Austria, por ejemplo, es menos costosa que una llamada de Estados Unidos a Austria.
  
### <a name="translating-outbound-dial-strings"></a>Convertir cadenas de marcado salientes

Skype para Business Server requiere que todas las cadenas que se normalice a formato E.164 con el fin de realizar la búsqueda de número inversa (RNL) de marcado. Para troncos con puertas de enlace o privada (PBX) que requieren números traducidos en formatos de marcado local, Skype para Business Server le permite crear una o más reglas que ayudan en la manipulación del número llamado (es decir, solicitar URI) antes de enrutamiento que el tronco. Por ejemplo, podrías escribir una regla para quitar +44 del encabezado de la cadena de marcado y cambiarlo por 0144.
  
Con Skype para Business Server, es posible crear una o más reglas que ayudan en la manipulación del número de llamada antes de enviarlo a la troncal.
  
En la planificación de los troncos que asociar pares de puerta de enlace: puerto con pares de servidor: puerto de mediación, puede ser útil a los troncos de grupo con locales similares requisitos de marcado y, por tanto, reducir el número de reglas de traducción necesaria y el tiempo que se tarda en escribir en ellos.
  
### <a name="configuring-caller-id"></a>Configurar el identificador de llamada

Skype para Business Server proporciona una forma de manipular el identificador de llamadas para llamadas salientes. Por ejemplo, si una organización desea enmascarar las extensiones de marcado directo de empleados y reemplazarlas con el genérico corporativa o el número de departamento, un administrador puede hacerlo utilizando Skype para Business Server Control Panel para suprimir el identificador de llamadas y reemplazarlo con un identificador de llamador alternativo especificado. En la planificación de la lógica de enrutamiento, tenga en cuenta que los individuos, grupos, sitios deseará esta opción para:, incluso, para todos los empleados.
...
- Si un usuario no está habilitado para la Telefonía IP empresarial, el servidor aplica otra lógica de enrutamiento, según corresponda.
