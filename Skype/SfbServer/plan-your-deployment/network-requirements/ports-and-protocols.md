...
> [!NOTE]
> Cuando Skype Empresarial Server, se abren los puertos necesarios en el firewall Windows. Windows firewall ya debería estar ejecutándose en la mayoría de las aplicaciones normales, pero si no se usa Skype Empresarial Server funcionará sin él. 
  
Para obtener más información acerca de la configuración del firewall para componentes perimetrales, vea Escenarios del servidor perimetral [en Skype Empresarial Server 2015](../../plan-your-deployment/edge-server-deployments/scenarios.md). 
...
|Servidores front-end  |Skype Empresarial Server de conferencia de mensajería instantánea  |5062  |TCP  |Se usa para las solicitudes SIP entrantes de las conferencias de mensajería instantánea.  |
|Servidores front-end  |Skype Empresarial Server de conferencia web  |8057  |TCP (TLS)  |Se usa para escuchar las conexiones del Modelo de objetos compartidos persistentes (PSOM) desde un cliente.  |
|Servidores front-end  |Skype Empresarial Server de compatibilidad de conferencias web  |8058  |TCP (TLS)  |Se usa para escuchar las conexiones del modelo de objetos compartidos persistentes (PSOM) desde el cliente de Live Meeting y las versiones anteriores de Skype Empresarial Server.  |
|Servidores front-end  |Skype Empresarial Server de conferencia de audio y vídeo  |5063  |TCP  |Se usa para las solicitudes SIP entrantes de las conferencias de audio y vídeo (A/V).  |
|Servidores front-end  |Skype Empresarial Server de conferencia de audio y vídeo  |57501-65535  |TCP/UDP  |Intervalo de puerto de medios usado para conferencias de vídeo.  |
|Servidores front-end  |Skype Empresarial Server de compatibilidad web  |80  |HTTP  |Se usa para la comunicación entre los servidores front-end y los FQDN (direcciones URL usadas por los componentes web IIS) cuando no se usa HTTPS.  |
|Servidores front-end  |Skype Empresarial Server de compatibilidad web  |443  |HTTPS  |Se usa para la comunicación entre los servidores front-end y los FQDN de la granja de servidores web (direcciones URL usadas por los componentes web IIS).  |
|Servidores front-end  |Skype Empresarial Server de compatibilidad web  |8080  |TCP y HTTP  |Usado por los componentes web para el acceso externo.  |
...
|Servidores front-end que también ejecutan un servidor de mediación instalado  |Skype Empresarial Server de mediación  |5070  |TCP  |Lo usa el servidor de mediación para solicitudes entrantes desde el servidor front-end al servidor de mediación.  |
|Servidores front-end que también ejecutan un servidor de mediación instalado  |Skype Empresarial Server de mediación  |5067  |TCP (TLS)  |Se usa para solicitudes SIP entrantes desde la puerta de enlace RTC al servidor de mediación.  |
|Servidores front-end que también ejecutan un servidor de mediación instalado  |Skype Empresarial Server de mediación  |5068  |TCP  |Se usa para solicitudes SIP entrantes desde la puerta de enlace RTC al servidor de mediación.  |
|Servidores front-end que también ejecutan un servidor de mediación instalado  |Skype Empresarial Server de mediación  |5081  |TCP  |Se usa para solicitudes SIP salientes desde el servidor de mediación a la puerta de enlace RTC.  |
|Servidores front-end que también ejecutan un servidor de mediación instalado  |Skype Empresarial Server de mediación  |5082  |TCP (TLS)  |Se usa para solicitudes SIP salientes desde el servidor de mediación a la puerta de enlace RTC.  |
|Servidores front-end  |Skype Empresarial Server de uso compartido de aplicaciones  |5065  |TCP  |Se usa para las solicitudes de escucha SIP entrantes para compartir las aplicaciones.  |
|Servidores front-end  |Skype Empresarial Server de uso compartido de aplicaciones  |49152-65535  |TCP  |Intervalo de puerto de medios usado para compartir aplicaciones.  |
...
|Servidores front-end  |Skype Empresarial Server de estacionamiento de llamadas  |5075  |TCP  |Se usa para las solicitudes SIP entrantes de la aplicación Estacionamiento de llamadas.  |
|Servidores front-end  |Skype Empresarial Server de prueba de audio  |5076  |TCP  |Se usa para las solicitudes SIP entrantes del servicio de prueba de audio.  |
|Servidores front-end  |No aplicable  |5066  |TCP  |Se usa para la puerta de enlace 9-1-1 mejorado (E9-1-1) saliente  |
|Servidores front-end  |Skype Empresarial Server de grupo de respuesta  |5071  |TCP  |Se usa para las solicitudes SIP entrantes de la aplicación Grupo de respuesta.  |
|Servidores front-end  |Skype Empresarial Server de grupo de respuesta  |8404  |TCP (MTLS)  |Se usa para las solicitudes SIP entrantes de la aplicación Grupo de respuesta.  |
|Servidores front-end  |Skype Empresarial Server de directivas de ancho de banda  |5080  |TCP  |Lo usa el servicio de directiva de ancho de banda del tráfico TURN perimetral A/V para el control de admisión de llamadas.  |
|Servidores front-end  |Skype Empresarial Server de servidor de recurso compartido de archivos  |445   |SMB/TCP  | Se usa para recuperar la libreta de direcciones, el contenido de la reunión y otros elementos almacenados en el servidor de recurso compartido de archivos.  |
|Servidores front-end  |Skype Empresarial Server de directivas de ancho de banda  |448  |TCP  |Se usa para el control de admisión de llamadas por el Skype Empresarial Server de directivas de ancho de banda.  |
|Servidores front-end donde reside el almacén de administración central  | Skype Empresarial Server de agente de replicador maestro |445  |TCP  |Se usa para insertar datos de configuración del almacén de administración central en servidores que ejecutan Skype Empresarial Server.  |
...
|Directores  |Skype Empresarial Server de compatibilidad web  |80  |TCP  |Se usa para la comunicación inicial desde los directores a los FQDN de la granja de servidores web (direcciones URL usadas por los componentes web IIS). En condiciones normales, cambiará a tráfico HTTPS usando el puerto 443 y el tipo de protocolo TCP.  |
|Directores  |Skype Empresarial Server de compatibilidad web  |443  |HTTPS  |Se usa para la comunicación desde los directores a los FQDN de la granja de servidores web (direcciones URL usadas por los componentes web IIS).  |
...
|Servidores de mediación  |Skype Empresarial Server de mediación  |5070  |TCP  |Lo usa el servidor de mediación para solicitudes entrantes desde el servidor front-end.  |
|Servidores de mediación  |Skype Empresarial Server de mediación  |5067  |TCP (TLS)  |Se usa para solicitudes SIP entrantes desde la puerta de enlace RTC.  |
|Servidores de mediación  |Skype Empresarial Server de mediación  |5068  |TCP  |Se usa para solicitudes SIP entrantes desde la puerta de enlace RTC.  |
|Servidores de mediación  |Skype Empresarial Server de mediación  |5070  |TCP (MTLS)  |Se usa para solicitudes SIP desde los servidores front-end.  |
