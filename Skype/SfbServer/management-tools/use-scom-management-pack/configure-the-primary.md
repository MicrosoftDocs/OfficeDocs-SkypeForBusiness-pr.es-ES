...
Para aprovechar al máximo las capacidades incluidas en Skype para Business Server 2015 de supervisión de estado nuevo, primero debe designar un equipo para que actúe como el servidor de administración principal. A continuación, debe instalar System Center Operations Manager 2012 SP1 o R2 o System Center Operations Manager 2007 R2 en el equipo. Además, debe instalar primero una versión compatible de SQL Server para que funcione como la base de datos de back-end de Operations Manager.
  
Al instalar System Center Operations Manager, debe instalar todos los componentes de dicho producto, incluyendo:
...
> [!IMPORTANT]
> El "[Paquete redistribuible de Microsoft informe Visor 2010](https://www.microsoft.com/en-us/download/details.aspx?id=6442)" debe instalarse antes de instalar System Center Operations Manager 2012. 
...
Tenga en cuenta que puede tener un único servidor de administración raíz por Skype para la implementación de Business Server.
...
Puede ampliar las capacidades de System Center Operations Manager mediante la instalación de paquetes de administración: software que indica qué elementos de System Center Operations Manager puede supervisar cómo se deben supervisar los elementos y cómo deben desencadenarse alertas y registrados. Skype para Business Server 2015 incluye dos módulos de administración de System Center Operations Manager que proporcionan las siguientes capacidades:
  
- **El componente y el módulo de administración de usuario** (Microsoft.LS.2015.Monitoring.ComponentAndUser.mp) realiza un seguimiento de Skype para Business Server problemas registrados en los registros de sucesos, registrados por los contadores de rendimiento o registra en los registros de detalles de llamadas (CDR) o las bases de datos de calidad de la experiencia (QoE). Para problemas críticos, System Center Operations Manager puede configurarse para notificar inmediatamente a los administradores a través de correo electrónico, mensaje instantáneo o de mensajes SMS. (SMS o servicio de mensajes cortos, es la tecnología que se utiliza para enviar mensajes de texto desde un dispositivo móvil a otro.)
    
    > [!NOTE]
    >  Para obtener más información acerca de cómo configurar la notificación de Operations Manager, consulte [Configuración de notificación](http://go.microsoft.com/fwlink/p/?LinkID=268785&amp;amp;clcid=0x409). 
  
- **El módulo de administración supervisión activa** (Microsoft.LS.2015.Monitoring.ActiveMonitoring.mp) proactivamente pruebas clave Skype para los componentes de servidor de negocio, como iniciar una sesión en el sistema, intercambiar mensajes instantáneos o realizar llamadas a un teléfono que se encuentra en la red telefónica pública conmutada (PSTN ). Estas pruebas se realizan utilizando el Skype para Business Server transacciones sintéticas cmdlets. Por ejemplo, el cmdlet **Test-CsIM** se utiliza para simular una conversación entre un par de usuarios de prueba de mensajería instantánea. Si se produce un error en esta conversación simulada, se genera una alerta.
...
- **System Center Operations Manager** Con este método, se utilice el Administrador de operaciones para agregar supervisión de Skype para Business Server.
    
- **Shell de Operations Manager** Puede utilizar el Shell de Operations Manager para importar directamente o para solucionar los problemas que experimenta al importar módulos de administración mediante la consola de System Center Operations Manager.
...
3. En el panel de administración, haga clic en **Módulos de administración**y, a continuación, haga clic en **Importar módulos de administración**.
