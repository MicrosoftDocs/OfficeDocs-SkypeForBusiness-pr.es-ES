...
Si implementa Skype empresarial online como parte de Microsoft 365 o de Office 365, un entorno híbrido de Skype empresarial Server o Skype empresarial Cloud Connector Edition, tendrá que comprender la comunicación entre el cliente y los servidores de Skype empresarial, así como el flujo de llamadas para que pueda planificar, implementar, operar y solucionar problemas de sus servicios de Skype empresarial online.
...
1. Una conferencia de Skype empresarial se hospeda en la misma región en la que se ha alojado el organizador de la Conferencia. Esto se encuentra en la nube si el organizador es un usuario en línea o en un centro de recursos local si el organizador de la reunión es un usuario local.

2. El tráfico multimedia enviado desde un cliente a una conferencia hospedada siempre va al servidor donde se hospeda la Conferencia. Esto puede ser un servidor local en un centro de información que se administra o un servidor en línea en la nube. Sin embargo, siempre se usa un servidor perimetral para el flujo de medios en conferencias en línea.
...
En general, el tráfico de red atraviesa la conexión de ExpressRoute si un extremo se encuentra en la red y el otro extremo se encuentra en el centro de recursos de Microsoft 365 u Office 365. Esto incluirá el tráfico de señales entre el cliente y el servidor, el tráfico de medios usado durante las llamadas en conferencia o las llamadas de par a par que usan un servidor perimetral en línea.

El tráfico no atraviesa la conexión de ExpressRoute si ambos puntos de conexión se pueden comunicar directamente a través de Internet o si se encuentran en su red. Esto incluirá los medios para las llamadas de punto a punto, el tráfico de Internet destinado a una implementación local o el tráfico entre los servidores de Internet y los servidores perimetrales de Microsoft 365 u Office 365. Un ejemplo de esto sería un usuario que se une a una conferencia en línea desde un hotel.
...
Para las llamadas de punto a punto, el tráfico de medios siempre toma la ruta más directa a su destino. Sin embargo, el tráfico de señalización se redirige a un centro de recursos de Microsoft 365 u Office 365 donde se ha alojado el usuario en línea. Puesto que los dos usuarios se encuentran en la misma WAN y nada impide que los clientes se comuniquen directamente, los medios fluyen directamente entre ellos. El tráfico de señalización, para ambos usuarios, atraviesa la conexión de ExpressRoute que está destinada al centro de recursos de cada organización. Para mostrar el flujo de llamadas en este escenario, consulte este.
...
En el ejemplo de punto a punto, el tráfico multimedia siempre toma la ruta más directa a su destino. Sin embargo, para una conferencia en línea, el destino está en la nube. Esto significa que el tráfico multimedia de todos los usuarios que se unan a la Conferencia desde dentro de la red atravesará la conexión de ExpressRoute y el tráfico de señalización se desplazará a la nube. En el siguiente gráfico se muestra que tanto los medios como la señalización atravesarán la conexión de ExpressRoute para un usuario de su red y que recorrerán directamente a Internet los usuarios que están conectados a Internet desde fuera de la red, como desde una cafetería o un hotel.

Recuerde que la ubicación de una conferencia está definida por el organizador de la reunión y no por los participantes. Esto significa que si la reunión ha sido programada por un cliente local, el tráfico multimedia no fluye a la nube en ExpressRoute, sino que, en cambio, atravesará Internet hasta el centro de proceso de reuniones local del organizador de la reunión.

El destino de los medios de las conferencias en línea será un centro de recursos de la nube de Microsoft 365 u Office 365, pero el centro de recursos puede estar en una región geográfica distinta de la de los usuarios que se están uniendo a la Conferencia. Esto puede ocurrir de dos maneras:
...
### <a name="on-premises-edge-server-with-microsoft-365-or-office-365-hosted-conferences"></a>Servidor perimetral local con Microsoft 365 u Office 365 conferencias hospedadas
<a name="bk_Figure5"> </a>

Cuando un usuario híbrido se une a una conferencia hospedada en línea, sabemos que la señalización y los medios se destinarán a la nube de Microsoft 365 u Office 365, y como el usuario se une desde Internet, normalmente se tomará una ruta de acceso a Internet directa. Sin embargo, en algunos casos, como por restricciones de firewall, no está disponible una ruta directa a través de Internet. En este caso, un servidor perimetral local puede retransmitir el tráfico multimedia, lo que hace que el tráfico multimedia vuelva a su red local antes de atravesar el circuito de ExpressRoute a la nube.
...
|Llamada de punto a punto  <br/> |Dos clientes, uno en su red (interno) y el otro cliente en Internet (externo).  <br/> |Usuario interno: ExpressRoute  <br/> Usuario externo: Internet  <br/> |Usuario interno: ExpressRoute  <br/> Usuario externo: Internet a Microsoft 365 u Office 365 Edge Server.  <br/> |[Llamada de punto a punto para usuarios de Microsoft 365 u Office 365 desde la red del cliente](call-flow-using-expressroute.md#bk_Figure2) <br/> |Supone que Firewall bloquea las conexiones directas entre clientes, que requieren un servidor perimetral en línea. El tráfico de usuarios internos a servidores perimetrales en línea sigue una ruta similar a la del servidor de conferencias para la llamada de conferencia.  <br/> |
...
|Unirse a la llamada de conferencia por usuario en la red del cliente  <br/> |Cliente de su servidor de conferencia y de red en la nube.  <br/> |ExpressRoute  <br/> |ExpressRoute  <br/> |[Usuario en línea de su red que se une a una conferencia hospedada en línea](call-flow-using-expressroute.md#bk_Figure3) <br/> ||
|Unirse a la llamada en conferencia por usuario en Internet  <br/> |El cliente está en Internet y en el servidor de conferencias en la nube.  <br/> |Internet  <br/> |Internet  <br/> |[Usuario en línea de su red que se une a una conferencia hospedada en línea](call-flow-using-expressroute.md#bk_Figure3) <br/> ||
...
|Llamada RTC  <br/> |Cliente en la red del cliente y servidores del sistema telefónico en la nube  <br/> |ExpressRoute  <br/> |ExpressRoute  <br/> |[Usuario en línea de su red que se une a una conferencia hospedada en línea](call-flow-using-expressroute.md#bk_Figure3) <br/> ||
|Llamada RTC  <br/> |Cliente en los servidores de sistema de Internet y teléfono en la nube  <br/> |Internet  <br/> |Internet  <br/> |No aplicable  <br/> |Los medios y la señalización fluyen al centro de distribución de Microsoft 365 u Office 365. Puesto que el punto de conexión del cliente está en Internet, todos los datos se transmiten al centro de datos de Microsoft a través de Internet (incluso si se necesita un servidor perimetral en línea para la conectividad).  <br/> |
