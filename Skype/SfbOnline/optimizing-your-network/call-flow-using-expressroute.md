...
El tráfico de red de Skype Empresarial Online se divide en dos amplias categorías, que se muestran como rutas separadas en el flujo de llamadas:

 **Los medios en tiempo real** son datos encapsulados dentro de RTP (protocolo de transporte en tiempo real) y admiten audio, vídeo, uso compartido de aplicaciones y cargas de trabajo de transferencia de archivos. En general, el tráfico multimedia es muy sensible a la latencia, por lo que sería mejor que este tráfico tomara la ruta más directa posible y que usara UDP como el protocolo de capa de transporte, ya que con el protocolo TCP se introduciría una latencia mayor.

 **La señalización** es el vínculo de comunicación entre el cliente y el servidor, u otros clientes que se usan para controlar las actividades (por ejemplo, cuando se inicia una llamada) y entregar los mi. La mayor parte del tráfico de señalización utiliza el protocolo SIP, aunque algunos clientes usan interfaces REST basadas en HTTP. Para que sea más sencillo, estamos considerando una variedad de señalizaciones que puedan viajar por conexiones HTTP y HTTPS o TLS en este tipo de tráfico. Es importante comprender que este tráfico es mucho menos sensible a la latencia, pero puede provocar cortes en el servicio o tiempos de espera en las llamadas si la latencia entre los puntos de conexión supera varios segundos.

Los destinos de este tráfico se encuentran en las DIRECCIONES URL e intervalos de direcciones IP de [Office 365](https://support.office.com/article/8548a211-3fe7-47cb-abb1-355ea5aa88a2) para todos los servicios de Microsoft 365 u Office 365. Para cada dirección URL, indica si esa parte del tráfico puede atravesar ExpressRoute para Microsoft 365 u Office 365. Para ver diagramas que muestran que Internet se sigue utilizando para algún tráfico cuando ExpressRoute está habilitado, consulte [Azure ExpressRoute para Office 365.](https://support.office.com/article/6d2534a2-c19c-4a99-be5e-33a0cee5d3bd) Es importante comprender que incluso las direcciones URL enumeradas como enrutables a través de ExpressRoute también se enrutan a través de Internet. Esto significa que, en algunos escenarios, determinar si se usará Internet o ExpressRoute depende de la ubicación del cliente y la configuración de los servidores proxy y los firewalls. También es importante comprender que, puesto que no todas las direcciones URL asociadas con Microsoft 365 u Office 365 pueden usar ExpressRoute, se requiere una conexión a Internet incluso si compra ExpressRoute en un partner de ExpressRoute.
...
1. Una conferencia de Skype Empresarial se hospeda en la misma región donde se aloja el organizador de la conferencia. Esto se encuentra en la nube si el organizador es un usuario en línea o en un centro de datos local si el organizador de la reunión es un usuario local.
...
Para obtener más información sobre los detalles de la ruta multimedia que se elige, consulte [ICE: conectividad multimedia perimetral.](https://aka.ms/AVEdge) Aunque este vídeo trata sobre Lync Server 2013, los principios y protocolos siguen a aplicarse en Skype Empresarial.

## <a name="skype-for-business-call-flows-with-expressroute"></a>Los flujos de llamada de Skype Empresarial con ExpressRoute

Ahora que ya conoce los cuatro segmentos de red diferentes y algunos principios generales de los flujos de llamada de Skype Empresarial, puede usar esa información para ayudarle a comprender qué tráfico de Skype Empresarial atravesará un segmento de red de ExpressRoute.
...
### <a name="peer-to-peer-call-for-microsoft-365-or-office-365-user-from-within-customer-network"></a>Llamada de punto a punto para usuario de Microsoft 365 u Office 365 desde dentro de la red del cliente
<a name="bk_Figure2"> </a>
...
El destino de los medios para las conferencias en línea será un centro de datos en la nube de Microsoft 365 u Office 365, pero el centro de datos puede encontrarse en una región geográfica diferente a la de los usuarios que se están uniendo a la conferencia. Esto puede suceder de dos maneras:
...
Cuando un usuario híbrido se une a una conferencia hospedada en línea, sabemos que la señalización y los medios se destinarán a la nube de Microsoft 365 u Office 365 y, como el usuario se une desde Internet, normalmente se tomaría una ruta de Internet directa. Sin embargo, en algunos casos, por ejemplo debido a las restricciones del firewall, no hay disponible una ruta de Internet directa. En este caso, un servidor perimetral local puede retransmitir el tráfico multimedia, lo que hace que el tráfico multimedia vuelva a su red local antes de atravesar el circuito de ExpressRoute hacia la nube.
...
|Llamada de punto a punto  <br/> |Dos clientes, ambos en su red.  <br/> |ExpressRoute  <br/> |Local  <br/> |[Llamada de punto a punto para usuario de Microsoft 365 u Office 365 desde dentro de la red del cliente](call-flow-using-expressroute.md#bk_Figure2) <br/> ||
|Llamada de punto a punto  <br/> |Dos clientes, uno en su red (interno) y el otro en Internet (externo).  <br/> |Usuario interno: ExpressRoute  <br/> Usuario externo: Internet  <br/> |Usuario interno: ExpressRoute  <br/> Usuario externo: De Internet a Microsoft 365 o al servidor perimetral de Office 365.  <br/> |[Llamada de punto a punto para usuarios de Microsoft 365 u Office 365 desde dentro de la red del cliente](call-flow-using-expressroute.md#bk_Figure2) <br/> |Asume que el firewall bloquea las conexiones directas entre clientes, que requiere un servidor perimetral en línea. El tráfico desde el usuario interno al servidor perimetral en línea sigue una ruta similar a la del servidor de conferencias para la llamada de conferencia.  <br/> |
...
|Unirse a la llamada de conferencia por usuario en la red del cliente  <br/> |El cliente en su red y el servidor de conferencias en la nube.  <br/> |ExpressRoute  <br/> |ExpressRoute  <br/> |[Usuario en línea de su red que se une a una conferencia hospedada en línea](call-flow-using-expressroute.md#bk_Figure3) <br/> ||
|Unirse a la llamada de conferencia por usuario en Internet  <br/> |El cliente está en Internet y en el servidor de conferencias en la nube.  <br/> |Internet  <br/> |Internet  <br/> |[Usuario en línea de su red que se une a una conferencia hospedada en línea](call-flow-using-expressroute.md#bk_Figure3) <br/> ||
