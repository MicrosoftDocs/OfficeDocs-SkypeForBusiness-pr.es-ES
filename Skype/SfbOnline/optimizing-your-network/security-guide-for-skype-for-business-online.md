...
### <a name="trustworthy-by-default"></a>Fiabilidad predeterminada
...
Una clave es un código o un número secreto que se usa para cifrar, descifrar o validar información secreta. Existen dos claves importantes en uso en la infraestructura de clave pública (PKI, Public Key Infrastructure) que deben ser consideradas: la clave privada que tiene cada titular de certificado y la clave de sesión, que se utiliza después de una identificación exitosa y de un intercambio de claves de sesión entre las partes que se estén comunicando.  Un ataque de clave comprometida se produce cuando el atacante averigua la clave privada o la clave de sesión. Cuando el atacante consigue la clave, puede usarla para descifrar datos cifrados sin que el remitente lo sepa.

Skype Empresarial Online usa las funciones PKI del sistema operativo Windows Server para proteger los datos de la clave que se usan para el cifrado de las conexiones TLS (Transport Layer Security). Las claves que se usan para cifrar elementos multimedia se intercambian mediante conexiones TLS. 

### <a name="network-denial-of-service-attack"></a>Ataque de denegación de servicio de red
El ataque por denegación de servicio se produce cuando el atacante impide que usuarios válidos usen la red de forma normal. Realizando un ataque por denegación de servicio, el atacante puede hacer lo siguiente:
- Enviar datos no válidos a las aplicaciones y los servicios que se ejecutan en la red atacada para interrumpir su funcionamiento normal.
...
- Impedir que los usuarios tengan acceso a los recursos de la red.
...
- Ha configurado conexiones compatibles solo con TCP (esto no se recomienda porque las comunicaciones TCP no están cifradas).
- Ha marcado las direcciones IP de esas conexiones como hosts de confianza. 

Este es un problema menor para las conexiones TLS (Seguridad de la capa de transporte), puesto que TLS autentica todas las partes y cifra todo el tráfico. Usar TLS evita que un atacante suplante las direcciones IP de una conexión concreta (por ejemplo, conexiones TLS mutuas). Sin embargo, un atacante aún podría suplantar la dirección del servidor DNS usado por SfBO. No obstante, puesto que en SfBO la autenticación se realiza con certificados, el atacante no tendrá un certificado válido necesario para suplantar a uno de los participantes de la comunicación.
...
### <a name="rtp-replay-attack"></a>Ataque de reply RTP
Un ataque de reply se produce cuando una transmisión de elementos multimedia válida entre dos partes se intercepta y se transmite con fines maliciosos. SfBO usa SRTP junto con un protocolo de señalización segura que protege las transmisiones de ataques de reply permitiendo que el receptor conserve un índice de los paquetes RTP ya recibidos y pueda comparar cada paquete nuevo con los que figuran en dicho índice.
...
Un virus es una unidad de código que tiene por objeto reproducir más unidades de código similares. Los virus necesitan un host —por ejemplo, un archivo, un correo electrónico o un programa— para poder funcionar. Al igual que un virus, un gusano es una unidad de código que está codificada para reproducir unidades de código adicionales similares, pero que, a diferencia de un virus, no necesita un host. Los virus y los gusanos suelen aparecer principalmente durante las transferencias de archivos entre clientes o cuando otros usuarios envían direcciones URL. Si su equipo tiene un virus, este podrá, por ejemplo, usar su identidad y enviar mensajes instantáneos en su nombre.  Las medidas estándar de seguridad de cliente como los análisis periódicos en busca de virus pueden mitigar este problema.  

### <a name="personally-identifiable-information"></a>Información de identificación personal
SfBO puede llegar a divulgar información a través de una red pública que podría vincularse a una persona. Dicha información se divide en dos categorías:
- **Datos de presencia mejorada**&nbsp;&nbsp;&nbsp;Los datos de presencia mejorada es información que el usuario tiene la opción de compartir mediante un vínculo a un socio federado o con contactos en una organización. Estos datos no se comparten con los usuarios de una red de mensajería instantánea pública. Las directivas de cliente y otras opciones de configuración del cliente pueden otorgar algún tipo de control al administrador del sistema. En el servicio SfBO se puede configurar el modo de privacidad de presencia mejorada para un usuario individual para evitar que los usuarios que no están en la lista de contactos del usuario vean la información de presencia del usuario. 
- **Datos obligatorios**&nbsp;&nbsp;&nbsp;Los datos obligatorios son datos que se requieren para el correcto funcionamiento del servidor o del cliente. Se trata de información necesaria en un servidor o en una red para el enrutamiento, el mantenimiento de estados y la señalización.
...
|Estado de presencia|Ausente, Disponible, Ocupado, No molestar, No conectado|
|||||
...
- La infraestructura de clave pública (PKI, Public Key Infrastructure) utiliza certificados emitidos por entidades de certificación (CA) de confianza para autenticar los servidores y garantizar la integridad de los datos.
- Transport Layer Security (TLS), HTTPS sobre SSL (HTTPS) y TLS mutua (MTLS) permiten la autenticación de los extremos y el cifrado de la mensajería instantánea. Las transmisiones de uso compartido y punto a punto de audio, video y aplicaciones están encriptadas y revisadas en cuanto a integridad con el protocolo de transporte seguro en tiempo real (SRTP).
- Para la autenticación del usuario se usan los protocolos estándar del sector, siempre que sea posible.
- Windows PowerShell proporciona funciones de seguridad habilitadas por defecto para que los usuarios no puedan ejecutar scripts fácilmente o sin darse cuenta.
- 
...
Las conexiones de servidor a servidor se basan en TLS mutua (MTLS) para autenticación mutua. En una conexión MTLS, el servidor de origen y de destino de un mensaje intercambian certificados procedentes de una CA de confianza para ambos. Los certificados permiten a los servidores probarse mutuamente sus identidades. El servicio SfBO sigue este procedimiento.

TLS y MTLS ayudan a evitar los ataques de interceptación y de tipo "Man in the middle". En los ataques de tipo "Man in the middle", el atacante redistribuye las comunicaciones entre dos entidades de red a través de su equipo sin que lo sepa ninguna de las entidades. La especificación de TLS y SfBO como servidores de confianza reduce parcialmente el riesgo de un ataque de tipo "Man in the middle" en la capa de aplicación. Lo consigue mediante el cifrado de extremo a extremo coordinado usando cifrado de claves públicas entre los dos extremos. Un atacante, para descifrar la comunicación, tendría que disponer de un certificado válido y de confianza, así como de la clave privada correspondiente, emitida a nombre del servicio con el que el cliente se esté comunicando. 
...
La confianza en un usuario se adjunta a cada mensaje que procede del usuario, no a la identidad del usuario. El servidor comprueba cada mensaje para determinar si las credenciales de usuario son válidas. Si las credenciales de usuario son válidas, el mensaje no es desafiado, no solo por el primer servidor que lo recibe, sino por el resto de servidores de SfBO.
...
Los participantes en la reunión también se clasifican por ubicación y credenciales. Puede utilizar estas dos características para especificar qué usuarios pueden tener acceso a reuniones específicas. En general, los usuarios pueden dividirse entre usuarios internos y externos:
- Los usuarios internos tienen credenciales de Azure Active Directory en la empresa.
- Los usuarios externos son aquellos que se conectan temporal o permanentemente a una empresa desde ubicaciones externas al cortafuegos corporativo. Pueden tener credenciales de Active Directory. Lync Server 2010 proporciona soporte de conferencia para los siguientes tipos de usuarios externos:
    - Usuarios remotos que tienen una identidad persistente de Active Directory dentro de la empresa. Pueden incluir empleados que trabajan desde casa o mientras viajan, y otros, como empleados de proveedores de confianza, a quienes se les han otorgado credenciales empresariales por sus términos de servicio. Los usuarios remotos pueden crear y unirse a conferencias y actuar como presentadores.
    - Los usuarios federados poseen credenciales válidas con socios federados y, por lo tanto, son tratados como autenticados por Lync Server 2010. Los usuarios federados pueden unirse a conferencias y ser promocionados a presentadores una vez que se hayan unido a la reunión, pero no pueden crear conferencias en las empresas en las que están federados.
    - Los usuarios anónimos no tienen una identidad de Active Directory y no están federados en la empresa. 

Los datos de cliente muestran que en muchas conferencias participan usuarios externos. Esos clientes también quieren confirmar la identidad de los usuarios externos antes de permitir que se unan a una conferencia. Como se describe en la siguiente sección, SfBO limita el acceso a la reunión a los tipos de usuarios permitidos y requiere que todos los tipos de usuario presenten las credenciales adecuadas al entrar en una reunión.
